...
export default function PaketlerIcerik() {
  const { t } = useLanguage();
  const p = t.paketler;
  ...
  return (
    <div style={{ background: "#0A0A0A", minHeight: "100vh", paddingTop: "80px" }}>
      ...
      <div className="sayfa-kap" style={{ paddingTop: "5rem", paddingBottom: "4rem" }}>
        ...
        <p style={{ fontFamily: "var(--font-dm-sans), sans-serif", fontSize: "1.25rem", color: "rgba(255,255,255,0.45)", maxWidth: "640px", lineHeight: 1.7, textWrap: "pretty" }}>
          {p.desc}
        </p>
      </div>

      {/* Kartlar */}
      <div className="sayfa-kap" style={{ paddingBottom: "5rem" }}>
        <style>{`
        ...
        <div className="paket-grid">
          {p.packages.map((pkg) => {
            ...
            return (
              <div key={pkg.id} style={{ background: bg, padding: "3rem 2.5rem", display: "flex", flexDirection: "column" }}>
                <p style={{ fontFamily: "var(--font-syne), sans-serif", fontWeight: 800, fontSize: "2.1rem", letterSpacing: "-0.02em", color: fg, marginBottom: "0.5rem" }}>
                  {pkg.name}
                </p>
                <p style={{ fontFamily: "var(--font-syne), sans-serif", fontWeight: 700, fontSize: "1.15rem", color: fg, marginBottom: "2.25rem", lineHeight: 1.25 }}>
                  {pkg.tagline}
                </p>
                ...
                <ul style={{ listStyle: "none", display: "flex", flexDirection: "column", gap: "0.85rem", marginBottom: "2.5rem", flex: 1 }}>
                  {pkg.features.map((f) => (
                    <li key={f} style={{ display: "flex", gap: "0.6rem", alignItems: "flex-start" }}>
                      ...
                      <span style={{ fontFamily: "var(--font-dm-sans), sans-serif", fontSize: "0.95rem", color: pkg.highlight ? "rgba(0,0,0,0.75)" : "rgba(255,255,255,0.65)", lineHeight: 1.5 }}>{f}</span>
                    </li>
                  ))}
                </ul>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.08)", borderBottom: "1px solid rgba(255,255,255,0.08)", padding: "5rem 0" }}>
        <div className="sayfa-kap">
          <p style={{ fontFamily: "var(--font-syne), sans-serif", fontWeight: 800, fontSize: "clamp(1.9rem, 3vw, 2.7rem)", color: "white", marginBottom: "0.75rem", letterSpacing: "-0.02em" }}>
            {p.tableTitle}
          </p>
          <p style={{ fontFamily: "var(--font-dm-sans), sans-serif", fontSize: "1.05rem", color: "rgba(255,255,255,0.35)", marginBottom: "3rem" }}>
            {p.tableDesc}
          </p>
                ...
                {p.table.map((grup) => (
                  <>
                    ...
                    {grup.satirlar.map((satir) => (
                      <tr key={satir.hizmet}>
                        <td style={{ fontFamily: "var(--font-dm-sans), sans-serif", fontSize: "0.95rem", color: "rgba(255,255,255,0.65)", padding: "0.9rem 1rem 0.9rem 0", borderBottom: "1px solid rgba(255,255,255,0.05)" }}>
                          {satir.hizmet}
                        </td>
                        {PKT_KEYS.map((key) => {
